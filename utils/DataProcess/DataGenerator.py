...
def gen_torch_dataset(data_configs, print_info=True):
    ...
    elif data_configs['name'] in ['solar']:
        data_array, date_array = csv_file_solar(data_configs, print_info)
    elif data_configs['name'] in ['tjg-1-20']:
        data_array, date_array = csv_file_tjg(data_configs, print_info)
        data_configs['period_type'] = 0
    else:
...
def csv_file_solar(data_configs, print_info):
    df = pd.read_csv(data_configs['raw_path'])
    date_array = df['date'].values
    data_array = df.drop(columns='date').values
    S,C = data_array.shape
...
def date_generate(data):
    # 使用split方法以逗号为分隔符，将字符序列拆分为一个字符串列表
    string_list = data.split(',')
    # 将字符串列表中的每个元素转换为整数
    year, month, day, hour, minute, second = map(int, string_list)
    # 利用这些整数创建一个datetime对象
    date_time_obj = datetime(year, month, day, hour, minute, second)
    # 使用strftime方法将datetime对象转换为所需的时间字符串格式
    time_string = date_time_obj.strftime("%Y-%m-%d %H:%M:%S")
    return time_string

def csv_file_tjg(data_configs, print_info):
    df = pd.read_csv(data_configs['raw_path'])
    df["date"] = pd.to_datetime(df["date"])
    #df["date"] = pd.to_datetime(df["date"].apply(date_generate))
    date_array = df['date'].values
    data_array = df.drop(columns=['date']).values
    S,C = data_array.shape
    data_array = data_array.reshape(S,1,C)
