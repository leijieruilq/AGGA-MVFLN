...
class fft_mlp(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch.fft.rfft(x)
        # np.save("x_real.npy",x.real.cpu().detach().numpy())
        # np.save("x_imag.npy",x.imag.cpu().detach().numpy())
        # np.save("x_r.npy",self.u_r(x.real).cpu().detach().numpy())
        # np.save("x_i.npy",self.u_i(x.imag).cpu().detach().numpy())
...
class stattn_layer(nn.Module):
    ...
    def forward(self,x):
        ...
        #return torch.einsum("bcit,cn->bnit",[x,adj]) 
        x = x.squeeze(dim=2)
