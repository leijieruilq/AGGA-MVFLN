...
class dynamic_adp(nn.Module):
    ...
    def forward(self,x):
        ...
        # np.save("mav_cemb.npy",self.c_emb.cpu().detach().numpy())
        adj = torch.einsum("ctd,ltm->clt",c_emb1,c_emb2)  #带有时间维度的动态自适应矩阵(c,c,t)
        adj = F.softmax(F.relu(adj),dim=-1)
        #return torch.einsum("bcit,cnt->bnit",[x,adj])
        x = torch.einsum("bcit,cnt->bnt",[x,adj]) #融合
        x = self.drop(x)
        w_w = torch.einsum("bct,lt->blt",x,self.w) 
        # np.save("mav_w.npy",w_w.cpu().detach().numpy())
        return w_w
...
class FreqConv(nn.Module):
    ...
    def forward(self, x1, x2, x3):# (t + p_d * (d_r-1) - k)/s + 1
