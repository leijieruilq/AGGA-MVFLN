...
class MAVModel(hyperparameters):
    def __init__(self, configs, **args):
        ...
        # self.mlp_in = nn.ModuleList([nn.Linear(self.s_in,self.s_in) for i in range(self.channels_emb_layers)])
        # self.mlp_out = nn.ModuleList([nn.Linear(self.s_out,self.s_out) for i in range(self.channels_emb_layers)])
    ...
    def forward(self, x):
        ...
        # for (layer,mlp) in zip(self.dadp_in,self.mlp_in):
        #     x_c = mlp(x_c)
        #     x_c = torch.einsum("bcnt,blt->blnt",[x_c,F.gelu(layer(x_c))]) + x_c
        ...
        # # for (layer,mlp) in zip(self.dadp_out,self.mlp_out):
        # #     y_c = mlp(y_c) 
        # #     y_c = torch.einsum("bcnt,blt->blnt",[y_c,F.gelu(layer(y_c))]) + y_c
